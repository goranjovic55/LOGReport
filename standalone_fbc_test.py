...
# Simple Node and NodeToken classes for testing
class NodeToken:
    def __init__(self, name, token_id, token_type, ip_address, port=23, protocol="telnet"):
        self.name = name
        self.token_id = str(token_id).strip()
...
class TestNodeManager:
    def __init__(self):
        self.nodes = {}
        self.log_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_logs")
        print(f"[DEBUG] Log root set to: {self.log_root}")
        self.config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nodes_test.json")
